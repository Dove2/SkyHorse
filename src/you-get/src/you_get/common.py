...
class SimpleProgressBar:
    ...
    def update(self):
        ...
        instance.currentSpeed = self.speed
        instance.currentProgress = percent
        instance.currentReceived = self.received
        instance.totalSize = self.total_size
        instance.totalPieces = self.total_pieces
    ...
    def current_progress(self):
        return round(self.received * 100 / self.total_size, 2)
...
def download_urls(
    ...
    instance.output_filename = output_filename
...
def any_download(url, **kwargs):
    print("any download", url)
    m, url = url_to_module(url)
    m.download(url, **kwargs)


def any_download_playlist(url, **kwargs):
    print("any download playlist", url)
    m, url = url_to_module(url)
...
def script_main(download, download_playlist, **kwargs):
    ...
    extractor_proxy = kwargs['extractor_proxy']
    info_only = kwargs['info']
    if kwargs['force']:
        force = True
    if kwargs['auto_rename']:
        auto_rename = True
    if kwargs['url']:
        dry_run = True
    if kwargs['json']:
    ...
    caption = True
    stream_id = kwargs['format'] or kwargs['stream'] or kwargs['itag']
    if kwargs['no_caption']:
        caption = False
    if kwargs['player']:
    ...
    try:
        ...
        download_main(
            ...
            URLs, kwargs['playlist'],
            output_dir=kwargs['output_dir'], merge=not kwargs['no_merge'],
